...
def test_lognormal_expected_percentiles_match_brute_force():
    ...
    df = generate(spec)
    lag_days = (df["load_time"] - df["event_time"]).dt.total_seconds() / 86_400
    expected = expected_days_to_percentiles(spec)["2024-01"]
    failures = []
    for pct in (50, 90, 95, 99):
        empirical = float(np.quantile(lag_days, pct / 100))
        # Tolerance: at N=200k the p99 sampling std-error is ~0.2 days on a ~32-day
        # value (<1%); 5% relative absorbs that plus the 1-second timestamp floor.
        if abs(empirical - expected[pct]) / expected[pct] > 0.05:
            failures.append(f"p{pct}: expected {expected[pct]:.2f}d, empirical {empirical:.2f}d")
...
def test_step_expected_percentiles_match_brute_force():
    ...
    for pct in (50, 90, 95, 99):
        empirical = float(np.quantile(lag_days, pct / 100))
        # Tolerance: the mixture CDF has slope >= min_fraction/month_len, so the
        # quantile std-error at N=150k is well under 0.2 days; 0.5 days absolute.
        if abs(empirical - expected[pct]) > 0.5:
            failures.append(f"p{pct}: expected {expected[pct]:.2f}d, empirical {empirical:.2f}d")
    assert not failures, "; ".join(failures)

...
def test_full_config_loads():
            ...
            tables=[
                ...
                minimal_table(
                    ...
                    event_time_via={
                        ...
                        "schema": "dbo",
                        "table": "referrals",
                        "join_on": [{"base_col": "referral_id", "lookup_col": "id"}],
    ...
    assert via.join_on[0].base_col == "referral_id"
    assert config.delivery.remotes[0].token_env == "DASHBOARD_PUSH_TOKEN"


def test_yaml_loader_expands_env_vars(tmp_path, monkeypatch):
...
def test_event_time_xor_event_time_via():
    ...
    via = {
        ...
        "schema": "dbo",
        "table": "lookup",
        "join_on": [{"base_col": "k", "lookup_col": "k"}],
        "column": "event_date",
    }
    with pytest.raises(ValidationError, match="exactly one of event_time or event_time_via"):
        ProbeConfig.model_validate(minimal_config(tables=[minimal_table(event_time_via=via)]))
...
def test_join_spec_requires_key_pairs():
    ...
    with pytest.raises(ValidationError, match="join_on"):
        ProbeConfig.model_validate(
            minimal_config(tables=[minimal_table(event_time=None, event_time_via=via)])
        )
...
def test_token_env_must_be_a_name_not_a_token():
                ...
                delivery={
                    "remotes": [
                        {"name": "origin", "url": "https://forge.example/d.git",
                         "token_env": "ghp_actual$token!value"}
                    ]
                }
            )
        )
...
def test_digest_is_stable_and_secret_redacted():
    base = ProbeConfig.model_validate(
        minimal_config(connection_url="mssql+pymssql://sa:password_one@localhost/demo")
    )
    same_but_password = ProbeConfig.model_validate(
        minimal_config(connection_url="mssql+pymssql://sa:password_two@localhost/demo")
    )
    other_host = ProbeConfig.model_validate(
        minimal_config(connection_url="mssql+pymssql://sa:password_one@127.0.0.1/demo")
    )
    ...
    # a password change must NOT change the digest (it is redacted before hashing)
    assert config_digest(base) == config_digest(same_but_password)
    assert config_digest(base) != config_digest(other_host)

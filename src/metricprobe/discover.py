...
def _yaml_str(value: str) -> str:
    """A JSON string literal is a valid, fully-quoted YAML scalar: identifiers
    named `yes`, `null` or containing ':' survive the round trip as strings."""
...
class ColumnInfo:
    ...
    def is_datetime(self) -> bool:
        ...
        if self.dialect == "mssql" and lowered in ("timestamp", "rowversion"):
            # T-SQL `timestamp` is a BINARY rowversion, not a point in time
            return False
        return lowered in DATETIME_TYPES or lowered.startswith(("timestamp", "datetime"))

    @property
    def resolution(self) -> str:
        return "date" if self.data_type.lower() == "date" else "datetime"
...
def draft_config(
    ...
    # probe_name uniqueness is a campaign-wide requirement: schema-qualify
    # whenever a table name repeats across schemas, then de-collide GLOBALLY
    # with a numeric suffix (schema-qualifying alone is not collision-safe:
    # alpha.orders and main.alpha_orders both map to alpha_orders_main)
    table_name_counts: dict[str, int] = {}
    for _, table in inventory:
        table_name_counts[table] = table_name_counts.get(table, 0) + 1
    used_probe_names: set[str] = set()

    def _unique_probe_name(table_schema: str, table: str) -> str:
        base = (
            f"{table_schema}_{table}_main" if table_name_counts[table] > 1 else f"{table}_main"
        )
        name, suffix = base, 2
        while name in used_probe_names:
            name = f"{base}_{suffix}"
            suffix += 1
        used_probe_names.add(name)
        return name
    ...
    lines = [
        ...
        f"connection_url: {_yaml_str(connection_url)}",
        "store:",
    ...
    for (table_schema, table), datetime_columns in sorted(inventory.items()):
        ...
        lines.append(f"    table: {_yaml_str(table)}")
        resolutions: list[tuple[str, str]] = []
        types_by_name = {c.column: c for c in datetime_columns}
        for role in ("event_time", "load_time"):
            picks = roles[role]
            if picks:
                others = (
                    f"  # other candidates: {_comment_safe(', '.join(picks[1:]))}"
                    if picks[1:]
                    else ""
                )
                lines.append(f"    {role}: {_yaml_str(picks[0])}{others}")
                resolutions.append((picks[0], types_by_name[picks[0]].resolution))
            else:
                pool = _comment_safe(", ".join(c.column for c in datetime_columns))
                lines.append(f"    {role}:  # FIXME — choose one of: {pool}")
        if resolutions:
            # the per-column resolution declaration (date | datetime) so a
            # coarser fallback is labelled correctly
            lines.append("    resolution:")
            for column, resolution in resolutions:
                lines.append(f"      {_yaml_str(column)}: {resolution}")
        for role in ("source_insert_time", "load_batch_col"):
            picks = [p for p in roles[role] if p not in (roles["load_time"][:1] or [])]
            if picks:
                lines.append(
                    f"    # {role}: {_comment_safe(picks[0])}  "
                    "# candidate — enable only if correct"
                )
    return "\n".join(lines) + "\n"

...
- **One orchestration state machine, executable in CI — and the campaign command is
  ...
  **PUBLISHED is claimed only after the actual delivery succeeded**, not after
  ...
  single-remote delivery). Delivery and its manifest record span independent
  systems, so the record is TWO-PHASE: everything fallible is STAGED before
  any push (a record failure aborts pre-publication with the remotes
  untouched), and only the finalize — an atomic rename — runs after the
  pushes. Multi-remote delivery cannot be transactional
  across independent git servers: a later push failure — or a failed
  record finalize after the pushes — triggers a
  ...
  residue recorded HONESTLY as `publish_partial` — never as PUBLISHED —
  regardless of remote count (the store and a git remote are independent
  systems even with one remote), and

...
- CLI: `run` executes ALL configured probes sequentially under ONE
  ...
  (a stub while Step 10 is skipped — it says so and points at `report`),
...
- Smoke tests: every figure builds for every scenario; zero external RESOURCE
  LOADS in the HTML plus a browser-enforced no-network CSP (the vendored
  Plotly.js source contains inert URL string literals — see CLAUDE.md);
...
- `examples/demo.py`: generate synthetic data → probe → report (the public demo;
  serve omitted — Step 10 is skipped, `report` is the interactive artifact).
...
- Tag v0.1.0 and successors (post-release review fixes tag v0.1.x; the
  supported pin is the LATEST tag, referenced by README and the workflow
  template). Until the PyPI job exists (phase 2), the supported install path
  is the git tag: `pip install git+https://github.com/<user>/metricprobe@<latest tag>`.
  Downstream deployments pin an exact tag.
